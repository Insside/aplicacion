...
$sql = "SELECT * FROM `aplicacion_framework_funciones`  WHERE(`clase`='".$clase."')  LIMIT " . $count . ",1;";
...
if ($configuraciones->modo == "desarrollo") {
  ...
  $result = $cliente->call("Framework_Funcion", array("funcion" => $fila));
  if ($error) {
...
$json["count"] = $count;
$json["other"] = true;
if (($json["count"]) >=($aff->conteo($clase)-1)) {
  $json["other"] = false;
}
