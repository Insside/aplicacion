/*
 ...
 ...
 */
MUI.iTextBox =new Class({
//var TextBox =new Class({
  Implements: [Events, Options],
  options: {
    //id:				'',			// id of the primary element, and id os control that is registered with insside
    ...
    cssClass: '', // Clase primaria aplicada
  ...
  initialize: function(options) {
    ...
    //InssideUI.set(this.id, this);

    if (options.maskType.capitalize() == 'Password')
  ...
  draw: function(container) {
    ...
    if (lbl)
      this.el.label = lbl.set('text', tle).set('for', o.id);
    ...
    if (!inp) {  // create input field if none given
      inp =new Element('input', {'type': 'input', 'id': o.id, 'name': o.id}).inject(fs);// Agrego name ya que en las transacciones se producen fallos.
    }
    this.el.input = inp.set('maxlength',o.maxlength).set('type', o.type).setStyle('width', o.width).set('class', o.cssClass);

    // determine value of input field
    var value = o.value;
    if (o.formData)
      value = MUI.getData(o.formData, (o.valueField ? o.valueField : o.id));	// get value from data if bound to a dataset
    if (!value)
    ...
    if (o.maskType != "none" && (!MUI.Mask || !MUI.Mask[o.maskType]) && this.el.input) {
      ...
      if (o.maskType == 'Password') {
        ...
        new MUI.Require({js: ["mask.js"],
          onload: function() {
            new MUI.Require({
              js: ['mask' + o.maskType.split('.')[0].toLowerCase() + '.js'],
              onload: function() {
                var o = this.options;
                var options = Object.clone(o.maskOptions);
                options.maskType = o.maskType.toLowerCase();
  ...
  getFormTitle: function() {
    ...
    if (o.formTitle)
      return o.formTitle;
    if (o.formTitleField)
      return MUI.getData(o.formData, o.formTitleField);
    if (o.formData)
      return MUI.getData(o.formData, o.id);
    return o.id;
  }
